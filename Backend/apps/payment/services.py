...
class PaymentService:
        ...
        try:
            ...
            if payment.subscription:
                ...
                SubscriptionHistory.objects.create(
                    ...
                    metadata={'payment_id': payment.id} # type:ignore
                )

            logger.info(f"Payment {payment.id} processed successfully") # type:ignore
            return True

        except Exception as e:
            logger.error(f"Error processing successful payment {payment.id}: {e}") # type:ignore
            return False
    ...
    def process_failed_payment(payment: Payment, reason: str = "") -> bool:
        ...
        try:
            ...
            logger.info(f"Payment {payment.id} marked as failed") # type:ignore
            return True

        except Exception as e:
            logger.error(f"Error processing failed payment {payment.id}: {e}") # type:ignore
            return False
    ...
    def cancel_subscription(subscription: Subscription) -> bool:
        ...
        try:
            ...
            logger.info(f"Subscription {subscription.id} cancelled") # type:ignore
            return True

        except Exception as e:
            logger.error(f"Error cancelling subscription {subscription.id}: {e}") # type:ignore
            return False
